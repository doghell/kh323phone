...
bool FFMPEGLibrary::Load()
{
  ...
  if (!DynaLink::Open("avcodec")
    ...
    TRACE(4,"H263\tFFLINK Failed to load a library, some codecs won't operate correctly;");
#if !defined(WIN32)
    TRACE(4,"H263\tput libavcodec.so in the current directory (together with this program) and try again");
#else
    TRACE(4,"H263\tput avcodec.dll in the current directory (together with this program) and try again");
#endif
    return false;
  }

  if (!GetFunction("avcodec_init", (Function &)Favcodec_init)) {
    TRACE(4,"H263\tFailed to load avcodec_int");
    return false;
  }

  if (!GetFunction("h263_encoder", (Function &)Favcodec_h263_encoder)) {
    TRACE(4,"H263\tFailed to load h263_encoder");
    return false;
  }

  if (!GetFunction("h263p_encoder", (Function &)Favcodec_h263p_encoder)) {
    TRACE(4,"H263\tFailed to load h263p_encoder");
    return false;
  }

  if (!GetFunction("h263_decoder", (Function &)Favcodec_h263_decoder)) {
    TRACE(4,"H263\tFailed to load h263_decoder");
    return false;
  }

  if (!GetFunction("register_avcodec", (Function &)Favcodec_register)) {
    TRACE(4,"H263\tFailed to load register_avcodec");
    return false;
  }

  if (!GetFunction("avcodec_find_encoder", (Function &)Favcodec_find_encoder)) {
    TRACE(4,"H263\tFailed to load avcodec_find_encoder");
    return false;
  }

  if (!GetFunction("avcodec_find_decoder", (Function &)Favcodec_find_decoder)) {
    TRACE(4,"H263\tFailed to load avcodec_find_decoder");
    return false;
  }

  if (!GetFunction("avcodec_alloc_context", (Function &)Favcodec_alloc_context)) {
    TRACE(4,"H263\tFailed to load avcodec_alloc_context");
    return false;
  }

  if (!GetFunction("avcodec_alloc_frame", (Function &)Favcodec_alloc_frame)) {
    TRACE(4,"H263\tFailed to load avcodec_alloc_frame");
    return false;
  }

  if (!GetFunction("avcodec_open", (Function &)Favcodec_open)) {
    TRACE(4,"H263\tFailed to load avcodec_open");
    return false;
  }

  if (!GetFunction("avcodec_close", (Function &)Favcodec_close)) {
    TRACE(4,"H263\tFailed to load avcodec_close");
    return false;
  }

  if (!GetFunction("avcodec_encode_video", (Function &)Favcodec_encode_video)) {
    TRACE(4,"H263\tFailed to load avcodec_encode_video");
    return false;
  }

  if (!GetFunction("avcodec_decode_video", (Function &)Favcodec_decode_video)) {
    TRACE(4,"H263\tFailed to load avcodec_decode_video");
    return false;
  }

  if (!GetFunction("avcodec_set_print_fn", (Function &)Favcodec_set_print_fn)) {
    TRACE(4,"H263\tFailed to load avcodec_set_print_fn");
    return false;
  }
  ...
  if (!GetFunction("avcodec_version", (Function &)Favcodec_version)) {
    TRACE(4,"H263\tFailed to get version");
    return false;
  }

  if (!GetFunction("avcodec_build", (Function &)Favcodec_build)) {
    TRACE(4,"H263\tFailed to get build");
    return false;
  }
  ...
  public:
    ...
    bool Read(RTPFrame & frame)
    {
      if (!frame.SetPayloadSize(hdr_size + data_size)) {
        //PTRACE(4, "H263Pck\tNot enough memory for packet of " << length << " bytes");
        return false;
      }
...
{ 
  ...
  if ((avcodec = FFMPEGLibraryInstance.AvcodecFindEncoder(CODEC_ID_H263)) == NULL) {
    TRACE(4, "H263\tCodec not found for encoder");
    return;
  }
  ...
  if (avcontext == NULL) {
    TRACE(4, "H263\tFailed to allocate context for encoder");
    return;
  }

  avpicture = FFMPEGLibraryInstance.AvcodecAllocFrame();
  if (avpicture == NULL) {
    TRACE(4, "H263\tFailed to allocate frame for encoder");
    return;
  }
...
int H263EncoderContext::EncodeFrames(const BYTE * src, unsigned & srcLen, BYTE * dst, unsigned & dstLen, unsigned int & flags)
{
  ...
  if (srcRTP.GetPayloadSize() < sizeof(PluginCodec_Video_FrameHeader)) {
    TRACE(4,"H263\tVideo grab too small, Close down video transmission thread.");
    return 0;
  }

  PluginCodec_Video_FrameHeader * header = (PluginCodec_Video_FrameHeader *)srcRTP.GetPayloadPtr();
  if (header->x != 0 || header->y != 0) {
    TRACE(4,"H263\tVideo grab of partial frame unsupported, Close down video transmission thread.");
    return FALSE;
  }
  ...
  if (encodedPackets.size() == 0) {
    TRACE(4, "H263\tEncoder internal error - there should be outstanding packets at this point");
    return 1;
  }

  dstLen = GetNextEncodedPacket(dstRTP, RTP_RFC2190_PAYLOAD, lastTimeStamp, flags);

  //TRACE(6, "H263\tEncoded " << srcRTP.GetPayloadSize() << " bytes of YUV420P raw data into " << dstRTP.GetSize() << " RTP frame(s)");
...
H263DecoderContext::H263DecoderContext()
{
  ...
  if ((avcodec = FFMPEGLibraryInstance.AvcodecFindDecoder(CODEC_ID_H263)) == NULL) {
    TRACE(4, "H263\tCodec not found for decoder");
    return;
  }
  ...
  avcontext = FFMPEGLibraryInstance.AvcodecAllocContext();
  if (avcontext == NULL) {
    TRACE(4, "H263\tFailed to allocate context for decoder");
    return;
  }

  picture = FFMPEGLibraryInstance.AvcodecAllocFrame();
  if (picture == NULL) {
    TRACE(4, "H263\tFailed to allocate frame for decoder");
    return;
  }

  if (!OpenCodec()) { // decoder will re-initialise context with correct frame size
    TRACE(4, "H263\tFailed to open codec for decoder");
    return;
  }

  frameNum = 0;

  TRACE(3, "H263\tDecoder created");
}
...
bool H263DecoderContext::OpenCodec()
{
  ...
  if (FFMPEGLibraryInstance.AvcodecOpen(avcontext, avcodec) < 0) {
    TRACE(4, "H263\tFailed to open H.263 decoder");
    return FALSE;
  }
...
void H263DecoderContext::CloseCodec()
{
  if (avcontext != NULL) {
    if (avcontext->codec != NULL) {
      FFMPEGLibraryInstance.AvcodecClose(avcontext);
      TRACE(5, "H263\tClosed H.263 decoder" );
    }
  }
}
